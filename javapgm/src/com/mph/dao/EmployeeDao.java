...
public class EmployeeDao {
	...
	public void type_scroll_sensitive_rs_insert()
	{
		con = MyDBConnection.getDBConnection();
		try {
			ps = con.prepareStatement("select eid,ename from mphemp where eid=11 ",ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_UPDATABLE);
			rs = ps.executeQuery();
	...
	public void batchUpdate()
	{
		...
		try {
			...
			st.addBatch("update mphemp set eid=13 where ename ='suk'");
			st.addBatch("update mphemp set ename ='Varunaa' where eid=3");
			st.addBatch("update mphemp set ename ='raama' where eid=10");
